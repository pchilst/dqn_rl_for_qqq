...
def backtester(atr_tp, atr_sl, short_reward_mult, neural_net, exp_rate, no_of_steps, pca_components, history_length):
    ...
    class StockEnv(gym.Env):
        ...
        def step(self, action):
            ...
            while future_day < len(self.data):
                ...
                adverse_move = max(0, self.entry_price - next_price_low)
                favorable_move = max(0, next_price_high - self.entry_price)
                ...
                if action == 0:  # Long position
                    if self.entry_price + self.atr_tp * self.entry_atr <= next_price_high or self.entry_price + self.atr_tp * self.entry_atr <= next_price_open:
                        tp_hit = True
                        break
                    elif self.entry_price - self.atr_sl * self.entry_atr >= next_price_low or self.entry_price - self.atr_sl * self.entry_atr >= next_price_open:
                        sl_hit = True
                        break
                elif action == 1: # Hold position
                    if self.entry_price - self.atr_sl * self.entry_atr >= next_price_low or self.entry_price - self.atr_sl * self.entry_atr >= next_price_open:
                        sl_hit = True
                        break
                    elif self.entry_price + self.atr_tp * self.entry_atr <= next_price_high or self.entry_price + self.atr_tp * self.entry_atr <= next_price_open:
                        tp_hit = True
                        break
    ...
    test_data.loc[(test_data['predictions'] == 0), 'Order'] = 'long'
    
    train_data.loc[(train_data['predictions'] == 0), 'Order'] = 'long'
    ...
    if finishing_vars[9] > 10 and finishing_vars[7] > .73 and finishing_vars[0] > 1000:
        os.chdir('./Saved Models from BT')
        filename = 'GPU_' + str(SEED) + '_long_v1_' + str(atr_tp) + '_' + str(atr_sl) + '_' + str(short_reward_mult) + '_' + str(neural_net) + '_' + str(exp_rate) + '_' + str(no_of_steps) + '_' + str(pca_components) + '_' + str(history_length)
...
CSV_NAME = 'GPU_' + str(SEED) + '_long_v1.csv'
